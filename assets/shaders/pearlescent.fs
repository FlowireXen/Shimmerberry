...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float dst = length(exp(-pnt * pnt));
    pnt *= rot(dst - cos(lng * 3.0 + time2));
    pnt *= rot(smoothstep(0.7, 0.2, abs(tan(atan(pnt.x, pnt.y)))));
    // Color-Distance
    float hlpr = 1.0 - clamp(0.5 + 0.5 * (dst - length(pnt)), 0.2, 1.0);
	dst = mix(dst, length(pnt), 0.866) - 0.7 * hlpr;
    ...
    // Adjust Blue-Value
    col.b = sqrt(col.b + dst * 0.25);
    // Gamma Correction
    col *= col * 0.3 + 0.7;
    tex.rgb *= 0.5 + 0.6;
    // Return
    return dissolve_mask(tex * vec4(col, 0.8), texture_coords, uv);
}
