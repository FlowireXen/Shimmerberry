SMODS.Joker {
	...
	loc_vars = function(self, info_queue, card)
		...
		info_queue[#info_queue + 1] = { key = "SEMBY_forced_perishable", set = "Other",
        ...
        if context.using_consumeable then
			...
			then
				...
				for _, consumable in ipairs(G.consumeables.cards) do
					...
					and not (consumable.ability.SEMBY_forced_perish_tally and consumable.ability.SEMBY_forced_perish_tally <= 1)
				...
				if #consumeables > 0 then
					...
					chaos_card.ability.SEMBY_forced_perish_tally = (chaos_card.ability.SEMBY_forced_perish_tally or G.GAME.perishable_rounds) - 1
					...
					G.E_MANAGER:add_event(Event({
						func = function()
							-- Apply Sticker
							chaos_card:add_sticker('SEMBY_forced_perishable', true)
							chaos_copy:add_sticker('SEMBY_forced_perishable', true)
							-- Emplace Copy
							...
							attention_text({
								major = chaos_card,
								text = ''..(chaos_card.ability.SEMBY_forced_perish_tally or G.GAME.perishable_rounds),
							...
							attention_text({
								major = chaos_copy,
								text = ''..(chaos_copy.ability.SEMBY_forced_perish_tally or G.GAME.perishable_rounds),
								scale = 0.8, hold = 1.4, backdrop_colour = G.C.IMPORTANT,
								align = 'cm', offset = { x = 0, y = 0 }
							})
							return true
